...
public class FFT implements IFft {
	...
	public static void main(String[] args) {
		...
		Complex[] c = f.cconvolve(x, x);
		f.show(c, "c = cconvolve(x, x)");

		// linear convolution of x with itself
		Complex[] d = f.convolve(x, x);
		f.show(d, "d = convolve(x, x)");
	}

	@Override
	public double[] doFFT(double[] real, int samples, int sample_rate) {
		// TODO Auto-generated method stub
		Complex[] x = new Complex[real.length];
		...
		double v[] = new double[real.length * 2];
		for (int i = 0; i < real.length; i++) {
			v[2 * i] = y[i].re();
			v[2 * i + 1] = y[i].im();
		}
